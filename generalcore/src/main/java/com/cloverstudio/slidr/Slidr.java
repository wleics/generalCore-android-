package com.cloverstudio.slidr;
...
import com.cloverstudio.slidr.widget.SliderPanel;

/**
 * This attacher class is used to attach the sliding mechanism to any
 * {@link Activity} that lets the user slide (or swipe) the activity
 * away as a form of back or up action. The action causes
 * {@link Activity#finish()} to be called.
 *
 *
...
public class Slidr {
	...
	/**
	 * Attach a slideable mechanism to an activity that adds the slide to
	 * dismiss functionality
	 *
	 * @param activity
	 *            the activity to attach the slider to
	 * @return a {@link com.cloverstudio.slidr.model.SlidrInterface} that allows the
	 *         user to lock/unlock the sliding mechanism for whatever purpose.
	 */
	...
	/**
	 * Attach a slideable mechanism to an activity that adds the slide to
	 * dismiss functionality and allows for the statusbar to transition between
	 * colors
	 *
	 * @param activity
	 *            the activity to attach the slider to
	 * @param statusBarColor1
	 *            the primaryDark status bar color of the interface that this
	 *            will slide back to
	 * @param statusBarColor2
	 *            the primaryDark status bar color of the activity this is
	 *            attaching to that will transition back to the statusBarColor1
	 *            color
	 *
	 * @return a {@link com.cloverstudio.slidr.model.SlidrInterface} that allows the
	 *         user to lock/unlock the sliding mechanism for whatever purpose.
	...
	public static SlidrInterface attach(final Activity activity, final int statusBarColor1, final int statusBarColor2) {
		...
		panel.setOnPanelSlideListener(new SliderPanel.OnPanelSlideListener() {
			...
			public void onSlideChange(float percent) {
				...
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP && statusBarColor1 != -1
						&& statusBarColor2 != -1) {
	 ...
	 * {@link com.cloverstudio.slidr.model.SlidrConfig}
	 *
	 * @param activity
	 *            the activity to attach the slider to
	 * @param config
	 *            the slider configuration to make
	 * @return a {@link com.cloverstudio.slidr.model.SlidrInterface} that allows the
	 *         user to lock/unlock the sliding mechanism for whatever purpose.
	 */
